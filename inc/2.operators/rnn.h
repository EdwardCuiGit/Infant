...
protected:
    ...
    RnnBase(uint input_dim, uint hidden_dim, uint output_dim, bool has_bias = true, bool hidden_as_output = false, TensorInit_Types init_type = TensorInit_Types::Gaussian,
            Activation_Types hidden_activation = Activation_Types::Tanh, Activation_Types output_activation = Activation_Types::Sigmoid)
        : Operator("RnnBase"), _hidden_as_output(hidden_as_output), _input_dim(input_dim), _hidden_dim(hidden_dim), _output_dim(output_dim), _has_bias(has_bias), _init_type(init_type),
...
public:
    RawRnn(uint input_dim, uint hidden_dim, uint output_dim, bool has_bias = true, TensorInit_Types init_type = TensorInit_Types::Gaussian,
        Activation_Types hidden_activation = Activation_Types::Tanh, Activation_Types output_activation = Activation_Types::Sigmoid)
        : RnnBase(input_dim, hidden_dim, output_dim, has_bias, false, init_type, hidden_activation, output_activation)
    {
        _w = create_param("w", {hidden_dim, input_dim}, init_type);
        _u = create_param("u", {hidden_dim, hidden_dim}, init_type);
        _v = create_param("v", {output_dim, hidden_dim}, init_type);

        if (_has_bias)
        {
            _b1 = create_param("b1", {hidden_dim}, init_type);
            _b2 = create_param("b2", {output_dim}, init_type);
        }
    }
...
public:
    Lstm(uint input_dim, uint hidden_dim, bool has_bias = true, TensorInit_Types init_type = TensorInit_Types::Gaussian,
         Activation_Types hidden_activation = Activation_Types::Tanh, Activation_Types output_activation = Activation_Types::Sigmoid)
        : RnnBase(input_dim, hidden_dim, hidden_dim, has_bias, true, init_type, hidden_activation, output_activation)
    {
        _wf = create_param("wf", {hidden_dim, input_dim}, init_type);
        _wi = create_param("wi", {hidden_dim, input_dim}, init_type);
        _wo = create_param("wo", {hidden_dim, input_dim}, init_type);
        _wc = create_param("wc", {hidden_dim, input_dim}, init_type);

        _uf = create_param("uf", {hidden_dim, hidden_dim}, init_type);
        _ui = create_param("ui", {hidden_dim, hidden_dim}, init_type);
        _uo = create_param("uo", {hidden_dim, hidden_dim}, init_type);
        _uc = create_param("uc", {hidden_dim, hidden_dim}, init_type);

        if (_has_bias)
        {
            _bf = create_param("bf", {hidden_dim}, init_type);
            _bi = create_param("bi", {hidden_dim}, init_type);
            _bo = create_param("bo", {hidden_dim}, init_type);
            _bc = create_param("bc", {hidden_dim}, init_type);
        }
    }
...
public:
    Gru(uint input_dim, uint hidden_dim, bool has_bias = true, TensorInit_Types init_type = TensorInit_Types::Gaussian,
        ...
        : RnnBase(input_dim, hidden_dim, hidden_dim, has_bias, true, init_type, hidden_activation, output_activation)
    {
        _wz = create_param("wz", {hidden_dim, input_dim}, init_type);
        _wr = create_param("wr", {hidden_dim, input_dim}, init_type);
        _wh = create_param("wh", {hidden_dim, input_dim}, init_type);

        _uz = create_param("uz", {hidden_dim, hidden_dim}, init_type);
        _ur = create_param("ur", {hidden_dim, hidden_dim}, init_type);
        _uh = create_param("uh", {hidden_dim, hidden_dim}, init_type);

        if (_has_bias)
        {
            _bz = create_param("bz", {hidden_dim}, init_type);
            _br = create_param("br", {hidden_dim}, init_type);
            _bh = create_param("bh", {hidden_dim}, init_type);
        }
    }
